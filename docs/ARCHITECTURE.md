# Архитектура AI PR Reviewer

Техническая документация: как сервис устроен внутри. Пользовательская
инструкция — в [`README.md`](../README.md). Ревьюер по умолчанию заточен под
Android, но фокус ревью задаётся профилем и настраивается под любое направление.
...
app/
  ...
  paths.py         # где лежат .env и profiles.d/ (репозиторий vs ~/.pr-reviewer)
...
## Где лежит конфиг (`paths.py`)

`.env` и `profiles.d/` резолвятся в зависимости от способа установки:

- **Источник/checkout** (`git clone`, `pip install -e .`): корень репозитория —
  чтобы `.env`, свои профили и `.github/workflows/` лежали рядом с кодом и ехали
  в CI. Детект: рядом с пакетом есть `pyproject.toml` и `app/`.
- **Установлен как утилита** (`pipx install`): `~/.pr-reviewer/` — пакет лежит в
  read-only site-packages, туда писать нельзя.

Override: `$PR_REVIEWER_HOME` (вся база) и `$PROFILES_DIR` (только профили).
Каталог создаётся по необходимости при записи `.env`/добавлении профиля.

## Установка / обновление / удаление

Способ установки определяет механику (`paths.is_source_checkout()`):

- **Репозиторий:** `install.sh` создаёт `.venv`, ставит пакет editable и кладёт
  симлинк `~/.local/bin/pr-reviewer` на venv-бинарь (у него абсолютный shebang —
  работает из любого каталога). `update` = `git pull --ff-only` + переустановка в
  `.venv`. `uninstall` снимает симлинк (только если он ведёт в этот проект) и по
  подтверждению удаляет `.env`; сам `.venv` убирает внешний `uninstall.sh` (нельзя
  удалить окружение «из-под себя»).
- **pipx:** `update` = `pipx upgrade ai-pr-reviewer` (fallback — переустановка из
  git). `uninstall` чистит `~/.pr-reviewer` (конфиг + профили) и подсказывает
  `pipx uninstall ai-pr-reviewer`.

Удаление симлинка сверяет `readlink` с `paths.package_root()` — чужой
`~/.local/bin/pr-reviewer` не трогаем.
